...
async function loadLeagues() {
  try {
    ...
    console.error("JSON Error: Using emergency backup leagues. Check your leagues.json file syntax!", err);
    // Emergency Backup so the page isn't blank
    leagues = {
...
function loadStats() {
  ...
  if (leagueKey === "nhl" || league.hasTOI) {
  ...
  // --- Start of Grouped Logic (NFL, MLB, etc.) ---
  const groups = {
  ...
  if (leagueKey === "tennis") {
    ...
    ["Game Won", "Game Loss", "Set Won", "Set Loss", "Ace", "Double Fault"].forEach(stat => {
      const pts = league.stats[stat];
      container.innerHTML += `<div class="stat-row"><div class="stat-label">${stat} — ${pts} pts</div><input type="text" class="stat-input" id="stat-${stat}" /></div>`;
    });
    return;
  }
...
function showExtraBreakdown(leagueKey) {
    ...
    if (leagueKey === "nba") {
      ...
      extraBox.innerHTML = `<h3>Single Stats</h3>Pts: ${pts}, Rebs: ${reb}, Asts: ${ast}<br>P+R+A = ${pts + reb + ast}`;
