...
function calculateScore() {
  ...
  if (leagueKey === "nascar") {
    ...
    if (!hideZero || fastest !== 0) {
      breakdown += `Fastest Laps: ${fastest} × 0.45 = ${(fastest * 0.45)}\n`;
      total += fastest * 0.45;
    }
    if (!hideZero || led !== 0) {
      breakdown += `Laps Led: ${led} × 0.25 = ${(led * 0.25)}\n`;
      total += led * 0.25;
    }
  ...
  stats.forEach(([label, points]) => {
    ...
    if (leagueKey === "mlb_pitcher") {
      if (label === "Innings Pitched") {
        ...
        breakdown += `${label}: ${val} IP (${outs} outs) = ${outs)}\n`;
        total += outs;
        return;
      }
  ...
  if (leagueKey === "mlb_pitcher" && innings >= 6 && earnedRuns <= 3) {
    ...
    breakdown += `Quality Start: 1 × ${qsPoints} = ${qsPoints}\n`;
    total += qsPoints;
  }
